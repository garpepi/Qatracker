		...
		if(!empty($data)){
			$this->db->trans_begin();
				try{
					...
					error_log('Error on insert users on insert_users method. Catch Exception trans_rollback.');
			...
			{
					$this->db->trans_rollback();
					error_log('Error on insert users on insert_users method. trans_status FALSE.');
					throw new Exception ('Error on insert');
			}
			else
			{
			...
			error_log('Error on insert users on insert_users method. $data is empty.');
			return false;
		}
    }
}